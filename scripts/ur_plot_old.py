...
def plot_results(args) -> None:
	...
	for subplot_index in range(len(args.x_axis)):
		...
		for exp_folder in args.exp_folder:
			...
			for folder in dirs:
				...
				data2 = load_data(folder, KEY_TIME, y_axis, args)
				if data is not None:
					...
					if data2 is not None :
						x2, y2 = data2
						x_data_agent2, y_data_agent2 = x_data_agent2 + [x2], y_data_agent2 + [y2]
		...
		if not empty_axis:
			...
			x_data_all2.append(x_data_agent2)
			y_data_all2.append(y_data_agent2)
	...
	for subplot_index in range(n_subplots):
		...
		x_data = x_data_all2[subplot_index][curve_index]
		label = all_labels[subplot_index][curve_index]

		ax2 = ax.twiny()
		plot_data(ax2, x_data, y_data, label)
...
def set_font_size():
	...
	plt.rc('axes', titlesize=FONT_SIZE_AXES_TITLE)    # fontsize of the axes title
	plt.rc('axes', labelsize=FONT_SIZE_AXES_LABEL)    # fontsize of the x and y labels
	plt.rc('xtick', labelsize=FONT_SIZE_AXES_TICK)    # fontsize of the tick labels
	plt.rc('ytick', labelsize=FONT_SIZE_AXES_TICK)    # fontsize of the tick labels
	plt.rc('legend', fontsize=FONT_SIZE_LEGEND)    # legend fontsize
	plt.rc('figure', titlesize=FONT_SIZE_FIGURE_TITLE)  # fontsize of the figure title
